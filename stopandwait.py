...
def send_reliable(cs, filedata, receiver_binding, win_size):
    ...
    while win_left_edge < INIT_SEQNO + content_len:
        ...
        rlist, wlist, xlist = select.select([cs], [], [], RTO)

        if rlist:
            data_from_receiver, receiver_addr = cs.recvfrom(100)
            ack_msg = Msg.deserialize(data_from_receiver)
            print('SUCCESS: received ACK {}'.format(str(ack_msg.ack)))
        else:
            print('ERROR: did not receive ACK by RTO... resending')
            curindex = seq_to_msgindex.get(win_left_edge)
            if curindex is None:
                lastkey, lastvalue = list(seq_to_msgindex.items())[-1]
                win_left_edge = lastkey
            else:
                previndex = curindex - 1
                prev = next((key for key, value in seq_to_msgindex.items() if value == previndex), None)
                win_left_edge = prev
