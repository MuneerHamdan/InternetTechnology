...
def send_reliable(cs, filedata, receiver_binding, win_size):
    ...
    while win_left_edge < final_ack:
        ...
        last_acked = transmit_entire_window_from(win_left_edge)


        while True:
            if win_left_edge > last_acked or win_left_edge > win_right_edge or first_to_tx > last_acked or first_to_tx > win_right_edge:
                first_to_tx = -1
                break
            ...
            if rlist:
                ...
                first_to_tx = ack_msg.ack
                if first_to_tx >= last_acked:
                    break
                rlist, _, _ = select.select([cs], [], [], RTO)
            else:
                if first_to_tx >= last_acked or first_to_tx == -1:
                    break
